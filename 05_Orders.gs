...
function _submitOrderInternal(body) {
  ...
  // here the customer has already paid on the HDFC-hosted page (this
  // submitOrder call happens AFTER hdfc_verifyReturn confirmed success).
  // Rejecting would leave the money taken on the gateway side without
  // an order in our sheet. Instead, accept the order and log a clear
  // warning so admin can cancel + refund it manually from the orders
  // tab. The customer-facing calendar greys closed days out anyway, so
  // this only happens if an admin closes a day during the narrow window
  // between checkout and HDFC's callback.
  ...
  for (const order of orders) {
    ...
    // Inflation_Surcharge (round(sub × 5%)). A single per-day round drifted ±₹1
    // on multi-meal days vs _calculateLoyaltyStreak, which sums per-row values.
    const submissionDaySurcharge = order.meals.reduce(
      (s, m) => s + Math.round((Number(m.subtotal) || 0) * 0.05), 0);
...
function _deleteOrderInternal(phone, rowId, refundType, opts) {
  ...
  if (pStatStr === "paid" || pStatStr === "wallet paid" || isOnAccountOrder) {
    ...
    // Mirrors submitOrder's DYNAMIC threshold (1 meal → ₹106, 2+ meals → ₹159):
    ...
    const oldThreshold = _mealsIn(sameDayRows.concat([r])) <= 1 ? 106 : 159;
    const remThreshold = _mealsIn(sameDayRows) <= 1 ? 106 : 159;
